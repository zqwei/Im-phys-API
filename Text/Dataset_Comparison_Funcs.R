...
verticalPieMD <- function(images, header, width="100%"){
  htmlMD <- paste0("<table style='width:", width, "'>")
  for (nImage in 1:(length(images)-1)){
    if(length(header)>1 || nchar(header)>0){
      htmlMD <- paste0(htmlMD, "<tr><th><center><br>", header[nImage],"</center></th><th></th></tr>")
    }
    htmlMD <- paste0(htmlMD, "<tr><td><center><img src=", images[nImage]," /></center></td>")
    htmlMD <- paste0(htmlMD, "<td><center><img src=", images[length(images)]," /></center></td></tr>")
  }
  htmlMD <- paste0(htmlMD, '</table>')
  cat(htmlMD)
}
...
includeVerticalPieMD <- function(ImageDir, Datasets = tmpDatasets, DatasetsName=tmpDatasetsName, fileType = 'svg'){
  Datasets <- c(Datasets, "Label")
  ImageDir <- paste0('../Plot/', ImageDir)
  DataFiles <- Datasets
  for (nData in 1:length(DataFiles)){
