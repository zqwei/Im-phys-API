...
for nData = 1:2
    ...
    DataSetList(4+nData) = DataSetList(nData);
    DataSetList(4+nData).name = ['FineTuned_LowNoise_' DataSetList(nData).name];
...
%                                             noNoise(randpermLargeK(numTrial, size(nDataSet(nUnit).unit_no_error, 1)), :);
    ...
    save([TempDatDir DataSetList(4+nData).name '.mat'], 'nDataSet');     
end
