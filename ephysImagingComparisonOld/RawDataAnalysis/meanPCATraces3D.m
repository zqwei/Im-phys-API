...
for nData              = [1 3 4]
    ...
    figure;
    hold on
    plot3(squeeze(pcaFiringRatesAverage(1, 1, :)), squeeze(pcaFiringRatesAverage(2, 1, :)), squeeze(pcaFiringRatesAverage(3, 1, :)), '-b', 'linewid', 2);
    plot3(squeeze(pcaFiringRatesAverage(1, 2, :)), squeeze(pcaFiringRatesAverage(2, 2, :)), squeeze(pcaFiringRatesAverage(3, 2, :)),   '-r', 'linewid', 2);
    hold off
    box off
    xlabel('PC1 score')
    ylabel('PC2 score')  
    ...
    setPrint(8*3, 6, [PlotDir 'CollectedUnitsPCA/CollectedUnitsPCATrace3D_' DataSetList(nData).name])
