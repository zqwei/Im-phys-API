...
if ~exist([PlotDir 'SingleUnitsNormalizedDiff'],'dir')
    mkdir([PlotDir 'SingleUnitsNormalizedDiff'])
end
...
setPrint(16, 6*2*2, [PlotDir 'SingleUnitsNormalizedDiff/SingleUnitsNormalizedDiff_' DataSetList(1).name], 'pdf')

for nData = 2:length(DataSetList)-1    
    load([TempDatDir DataSetList(nData).name '.mat'])
    plotCaSelectivityIndex(nDataSet, wholeTrialStartingTime, wholeTrialEndTime, DataSetList(nData).params);    
    setPrint(16, 6*2*2, [PlotDir 'SingleUnitsNormalizedDiff/SingleUnitsNormalizedDiff_' DataSetList(nData).name], 'pdf')
end
