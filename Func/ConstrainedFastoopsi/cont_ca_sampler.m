...
s_1 = sparse(ceil(spiketimes_/Dt),1, double(exp((spiketimes_ - Dt*ceil(spiketimes_/Dt))/tau(1))),T,1);  
s_2 = sparse(ceil(spiketimes_/Dt),1, double(exp((spiketimes_ - Dt*ceil(spiketimes_/Dt))/tau(2))),T,1);  
...
for i = 1:N
    ...
    s_1 =   sparse(trunc_spikes,1, double(exp((spiketimes_ - Dt*trunc_spikes)/tau(1))),T,1);
    s_2 =   sparse(trunc_spikes,1, double(exp((spiketimes_ - Dt*trunc_spikes)/tau(2))),T,1);  
    if p == 1; G1sp = zeros(T,1); else G1sp = G1\s_1(:); end
    Gs = (-G1sp+G2\s_2(:))/diff(gr);
    ss{i} = spiketimes;
    ...
    if ~marg_flag
        ...
        [temp,~, SAMPLES.hmc_reject(i)] = HMC_exact2(eye(3), -lb, L, mu_post, 1, Ns, x_in);
        Am(i) = temp(1,Ns);
        Cb(i) = temp(2,Ns);
        Cin(i) = temp(3,Ns)';
        A_ = Am(i);
    ...
    if gam_flag
        ...
        %        prior_ratio = gampdf(tau_(2),12,1)/gampdf(tau(2),12,1);
            ...
            tau_(2) = tau_temp;
            s_2_ =   sparse(trunc_spikes,1, double(exp((spiketimes_ - Dt*trunc_spikes)/tau_(2))),T,1);  
            gr_ = exp(Dt*(-1./tau_));
            ge_ = max(gr_).^(0:T-1)';
...
SAMPLES.Am = Am(B+1:N);
SAMPLES.hmc_reject = SAMPLES.hmc_reject(B+1:N);
if gam_flag
    SAMPLES.g = Gam(B+1:N,:);
end
