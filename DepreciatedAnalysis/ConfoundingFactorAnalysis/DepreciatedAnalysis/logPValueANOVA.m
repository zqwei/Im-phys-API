...
setPrint(8, 6, [PlotDir 'ConfoundingFactorAnova/SingleUnitsAnovaAPML_Pop_' DataSetList(nData).name])

APLoc          = [DataSetList(nData).cellinfo.AP_axis];
MLLoc          = [DataSetList(nData).cellinfo.ML_axis];
figure;
sizeGroup      = histcounts(unitGroup(APLoc>2400 & APLoc<2600 & MLLoc>1100 & MLLoc<1900), 1:5);
sum(sizeGroup)
h = pie(sizeGroup);
h = findobj(h, 'Type', 'patch');
for nh = 1:length(h)
...
setPrint(8, 6, [PlotDir 'ConfoundingFactorAnova/SingleUnitsAnovaAPML_Sub_' DataSetList(nData).name])
...
setPrint(8*2, 6, [PlotDir 'ConfoundingFactorAnova/SingleUnitsAnovaANM_' DataSetList(nData).name])
