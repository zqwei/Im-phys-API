...
function noiseCorrelationEpochs
    ...
    for nData   = 1:length(DataSetList)
        ...
        for nPlot = 1:3
            ...
            hold on
            plotData = [yesTrialSignalCorr(yes_no_pair(:, 1), nPlot), noTrialSignalCorr(yes_no_pair(:, 2), nPlot)];
            [N, C] = hist3(plotData, [30, 30]);
            N(N==0) = nan;
            b = imagesc(C{1}, C{2}, log10(N/length(yes_no_pair)));
            set(b,'AlphaData',~isnan(N));
            colorbar;
            axis xy;
            plot([-1 1], [-1 1], '--k', 'linewid', 1.0)
            plot([1 -1], [-1 1], '--k', 'linewid', 1.0)
            plot([0 0], [-1 1], '--k', 'linewid', 1.0)
            plot([-1 1], [0 0], '--k', 'linewid', 1.0)
            ...
            hold on
            plotData = [yesTrialNoiseCorr(yes_no_pair(:, 1), nPlot), noTrialNoiseCorr(yes_no_pair(:, 2), nPlot)];
            [N, C] = hist3(plotData, [30, 30]);
            N(N==0) = nan;
            b = imagesc(C{1}, C{2}, log10(N/length(yes_no_pair)));
            set(b,'AlphaData',~isnan(N));
        ...
        setPrint(8*3, 6*4, [PlotDir 'C2SModel/SingleUnitsNoiseCorrelation_' DataSetList(nData).name])
        setPrint(8*3, 6*4, [PlotDir 'C2SModel/SingleUnitsNoiseCorrelation_' DataSetList(nData).name],'png')
