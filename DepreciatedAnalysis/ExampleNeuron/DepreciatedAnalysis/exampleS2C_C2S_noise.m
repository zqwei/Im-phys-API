...
for nCell = 1:length(nDataSet)
    ...
    allData       = [nCellDataSet.unit_yes_trial_linear; nCellDataSet.unit_no_trial_linear];
    stdData       = std(allData(:));
    yesNoise      = randn(size(nCellDataSet.unit_yes_trial_linear)) * stdData * 2;
    noNoise       = randn(size(nCellDataSet.unit_no_trial_linear)) * stdData * 2;
...
save('S2CC2SNoise.mat', 'nDataSet');

load('S2CC2SNoise.mat', 'nDataSet')
