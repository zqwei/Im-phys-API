...
function SpikeDataSet = getSpikeDataWithEphysTimeAndBoxCar(SpikingDataDir, SpikeFileList, minNumTrialToAnalysis, TimeToAnalysis, totbinSize)
    ...
    for nfile = 1:length(SpikeFileList)
        ...
        for nUnit           = 1:numUnits
            ...
            if sum_valid_yes> minNumTrialToAnalysis && sum_valid_no> minNumTrialToAnalysis
                unit_yes_trial = nan(sum_valid_yes, length(psthStart:psthEnd));
                unit_no_trial  = nan(sum_valid_no, length(psthStart:psthEnd));
                ...
                for n_trial  = 1: sum_valid_yes
                    ...
                    unit_trial         = unit_trial(unit_trial>min(psthTime) ...
                                         & unit_trial < max(psthTime));
                    unit_data          = conv(hist(unit_trial,psthTime)/binSize, boxCarWindow, 'same');
                    unit_yes_trial(n_trial,:) = unit_data(psthStart:psthEnd);
                ...
                for n_trial  = 1: sum_valid_no    
                    ...
                    unit_trial         = unit_trial(unit_trial>min(psthTime) ...
                                         & unit_trial < max(psthTime));
                    unit_data          = conv(hist(unit_trial,psthTime)/binSize, boxCarWindow, 'same');
                    unit_no_trial(n_trial,:) = unit_data(psthStart:psthEnd);
                ...
                sum_valid_yes       = sum(n_valid_yes);
                sum_valid_no        = sum(n_valid_no);
                unit_yes_error      = nan(sum_valid_yes, length(psthStart:psthEnd));
                unit_no_error       = nan(sum_valid_no, length(psthStart:psthEnd));
                ...
                for n_trial  = 1: sum_valid_yes
                    ...
                    unit_trial         = unit_trial(unit_trial>min(psthTime) ...
                                         & unit_trial < max(psthTime));
                    unit_data          = conv(hist(unit_trial,psthTime)/binSize, boxCarWindow, 'same');
                    unit_yes_error(n_trial,:) = unit_data(psthStart:psthEnd);
                ...
                for n_trial  = 1: sum_valid_no    
                    ...
                    unit_trial         = unit_trial(unit_trial>min(psthTime) ...
                                         & unit_trial < max(psthTime));
                    unit_data          = conv(hist(unit_trial,psthTime)/binSize, boxCarWindow, 'same');
                    unit_no_error(n_trial,:) = unit_data(psthStart:psthEnd);
