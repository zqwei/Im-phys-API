...
for nData                     = 1:length(fileList)
    ...
    numUnits                  = length(nDataSet(ephysCellIndex{nData}));
    pValue                    = applyFuncToCompareTrialType(nDataSet(ephysCellIndex{nData}), @pValueTTest2, filterInUse);
    meanDiffValue             = applyFuncToCompareTrialType(nDataSet(ephysCellIndex{nData}), @meanDiff, filterInUse);
    ...
    ML_axis                      = [DataSetList(nData).cellinfo(ephysCellIndex{nData}).ML_axis];
    AP_axis                      = [DataSetList(nData).cellinfo(ephysCellIndex{nData}).AP_axis];
    depth                        = [DataSetList(nData).cellinfo(ephysCellIndex{nData}).depth];
    
    subplot(length(fileList), 4, 4*(nData -1) + 2)
    hold on;
    iGroup                       = floor((ML_axis-1100)/200); % 0 1 2 3
    ...
    for nGroup                   = 0:3
        ...
        plot(DataSetList(nData).params.timeSeries(4:3:end-3), nCell/sum(nCell));
    ...
    plot(depth(bumpStartPoint<numT), startTimePoint(bumpStartPoint<numT), 'o');
    xlim([110 750])
    ylim([DataSetList(nData).params.timeSeries(1) DataSetList(nData).params.timeSeries(end)])
    ylabel('First Sig. p Val. (s)')
    xlabel('Depth (um)')
    box off
end

setPrint(4*8, length(DataSetList)*6, [PlotDir 'All_Data_Summary/Cropped_Data_Summary'], 'pdf')