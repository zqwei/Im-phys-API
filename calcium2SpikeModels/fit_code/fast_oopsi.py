...
@revision by: Ziqiang Wei <weiz AT janelia DOT hhmi DOT com>
@date: 2014-04-16
"""
import numpy as np
...
def fast(F, dt=0.02, iter_max=1, update=True):
    ...
    for i in xrange(1, iter_max):
        ...
        if np.abs((ml[i]-ml[i-1])/ml[i]) < 1e-3 or any(np.abs(ml[:i]-ml[i]) < 1e-5):
            break
    # estimate of Fluroscense
    F_est = P_best['alpha']*C_best + P_best['beta']
    F_est = F_est * (F_max - F_min) + F_min + deltaF
    return n_best, C_best, P_best, F_est
...
def wiener(F, dt=0.020, iter_max=20, update=True):
    ...
    T = F.shape[0]
    gam = 1.0 - dt/1.0
    M = spdiags([-gam*np.ones(T), np.ones(T)], [-1, 0], T, T, format='csr')
    C = np.ones(T)
    n = M*C
    lam = 1.0
    llam = (lam*dt)
    ...
    for i in xrange(iter_max):
        ...
        if lik <= old_lik - gtol:  # NR step decreases likelihood
            ...
            break
    # adding removing negative firing in code
    n[n<0.] = 0.
    F_est = C * F_max + F_mean
    C = linsolve.spsolve(M, n)
    F_est_nonneg = C * F_max + F_mean
    n = n/n.max()
    return n, C, F_est, F_est_nonneg
...
def discretize(F, bins=[0.12], high_pass=True):
    ...
    vmax = v.max() + epsilon
    vmin = v.min() - epsilon
    D = np.zeros(F.shape);  # pre-allocate the storage (do we need?)
    if np.isscalar(bins):
