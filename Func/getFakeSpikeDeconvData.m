...
function DataSetOOPSI = getFakeSpikeDeconvData(spikeDataSet, tau_r, tau_d, params)
    ...
    for nData          = 1:length(spikeDataSet)     
        ...
        fastData                                 = imagingToSpikeDeconv(spikeDataSet(nData).unit_yes_trial, tau_r(nData), tau_d(nData), params);
        DataSetOOPSI(nData).unit_yes_trial       = fastData;
        %% unit_no_trial
        DataSetOOPSI(nData).unit_no_trial_index  = spikeDataSet(nData).unit_no_trial_index;
        fastData                                 = imagingToSpikeDeconv(spikeDataSet(nData).unit_no_trial, tau_r(nData), tau_d(nData), params);
